...
function ampRender() {
    ...
    area.innerHTML = `
        <style>
            .amp-solta {
                border: 2px dashed var(--border-strong, #cdd5e1); border-radius: 10px; padding: 28px 18px; text-align: center;
                background: var(--surface-2, #f6f8fb); transition: all .15s; cursor: pointer;
            }
            .amp-solta.amp-sobre { border-color: var(--accent, #2563c9); background: var(--accent-soft, #edf3fd); }
            .amp-opcao {
                border: 1px solid var(--border, #e3e8ef); background:var(--surface, #fff); border-radius: 8px; padding: 7px 12px;
                cursor: pointer; font-size: 13px; color: var(--text-2, #3d4759); transition: all .15s;
            }
            .amp-opcao:hover { border-color: var(--text-4, #7a869a); }
            .amp-opcao.amp-ativa { border-color: var(--accent, #2563c9); background: var(--accent-soft, #edf3fd); color: var(--accent-ink, #1f55ad); font-weight: bold; }
            .amp-opcao[disabled] { opacity: .45; cursor: not-allowed; }
            .amp-rotulo { font-size: 12px; font-weight: bold; color: var(--text-2, #3d4759); text-transform: uppercase; letter-spacing: .03em; }
        ...
        <div class="card" style="margin:20px 0;">
            ...
            ${ampAviso ? `<div style="background:var(--accent-soft, #edf3fd); border:1px solid #a9c6f3; color:#1b4488; border-radius:8px; padding:10px 14px; margin:12px 0; font-size:13px; line-height:1.6;">
                ${escAmp(ampAviso)}</div>` : ''}
...
function ampHtmlSemImagem() {
    return `
        <div class="amp-solta" onclick="document.getElementById('ampArquivo').click()"
            ...
            <div style="font-weight:bold; color:var(--text, #1c2536); margin-top:10px;">Escolha a imagem ou arraste ela para cá</div>
            <div style="color:var(--text-3, #5f6b7f); font-size:12px; margin-top:6px;">
...
function ampHtmlComImagem() {
    ...
    return `
        <div style="display:flex; gap:14px; align-items:center; flex-wrap:wrap; background:var(--surface-2, #f6f8fb);
                    border:1px solid var(--border, #e3e8ef); border-radius:8px; padding:10px 12px; margin:12px 0;">
            <img src="${escAmp(img.url)}" alt="" style="width:64px; height:64px; object-fit:contain; background:var(--surface, #fff); border:1px solid var(--border, #e3e8ef); border-radius:6px;">
            <div style="flex:1; min-width:180px;">
                <div style="font-weight:bold; color:var(--text, #1c2536); font-size:14px; word-break:break-all;">${escAmp(img.nome)}</div>
        ...
        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(230px, 1fr)); gap:16px 22px; margin:18px 0;">
            ...
            <div>
                ...
                ${ampOpcoes.formato === 'image/png' ? `
                    ...
                    <div style="margin-top:8px;">
                        <label style="color:var(--text-3, #5f6b7f); font-size:12px; display:block; margin-bottom:2px;">
        ...
        <div style="display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
            ...
            ${escala === 1 && !ampOcupado ? '<span style="color:var(--text-3, #5f6b7f); font-size:12px;">Em 1x a imagem só é limpa e realçada, sem aumentar.</span>' : ''}
        </div>
...
function ampHtmlProgresso() {
    ...
    return `
        <div style="background:var(--surface-3, #eef2f7); border-radius:999px; height:10px; overflow:hidden;">
            <div style="width:${pct}%; height:100%; background:var(--accent, #2563c9); transition:width .2s;"></div>
        </div>
        <div style="font-size:12px; color:var(--text-3, #5f6b7f); margin-top:6px;">${escAmp(ampProgresso.texto || '')} ${pct}%</div>`;
}
        ...
        <div style="margin-top:18px; border-top:1px solid var(--border, #e3e8ef); padding-top:16px;">
            <div style="display:flex; gap:12px; align-items:center; flex-wrap:wrap; margin-bottom:12px;">
                <div style="flex:1; min-width:200px;">
                    <div style="font-weight:bold; color:var(--text, #1c2536);">✅ Pronto — ${r.largura} × ${r.altura} px</div>
                    <div style="color:var(--text-3, #5f6b7f); font-size:12px; margin-top:2px;">
            ...
            <div style="display:flex; justify-content:space-between; color:var(--text-3, #5f6b7f); font-size:12px;">
